...
if __name__ == "__main__":
    ...
    fieldnames = ['package', 'issues', 'blocker_issues', 'critical_issues', 'major_issues', 'minor_issues', 'files_processed']
    ...
    run_lint_with_timeout = timeout_decorator.timeout(300)(run_lint)
    with click.progressbar(projects) as bar:
        for package, project_path in bar:
            if package in sonar_processed_packages:
                ...
                try:
                    results = run_lint_with_timeout(project_path)
                except timeout_decorator.TimeoutError:
                    results = None
                    click.secho("Skipping {}: timed out after 5 minutes.".format(package))
                if results:
